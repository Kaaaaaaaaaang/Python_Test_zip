...
from test5_question2_window import Test5_Question2_Window
...
class Test5_Question1_Window(QtWidgets.QMainWindow, QPushButton):
    ...
    def initUI(self):
        ...
        #질문
        self.q1 = QLabel("나는 혼밥, 혼영이 더 편하다. ", self)
        ...
        self.test5_question2_window = Test5_Question2_Window(self)

    def a_click(self):
        self.test5_question2_window.show()
        user_name = self.name_label.text()
        self.test5_question2_window.name_label.setText(user_name)

        print(user_name, ">>>점수 추가 완료")
        #print(self.name_label.text() + " : a 선택")
        if self.label.text() == "         's Love Cell Survival Test":
            self.test5_question2_window.label.setText("         's Love Cell Survival Test")
            self.test5_question2_window.q1.setText("It has been more than two years since I became a single. ")
        elif self.label.text() == "         恋愛細胞生存テスト":
            self.test5_question2_window.label.setText("         恋愛細胞生存テスト")
            self.test5_question2_window.q1.setText("シングルになってから2年過ぎた。")
        self.hide()
    ...
    def b_click(self):
        ...
        sql = "UPDATE lovesurvival_test SET calculate = 10 WHERE result = 0;"
        cursor.execute(sql)
        cnx.commit()
