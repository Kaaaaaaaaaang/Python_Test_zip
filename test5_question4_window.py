...
from test5_question5_window import Test5_Question5_Window
...
class Test5_Question4_Window(QtWidgets.QMainWindow, QPushButton):
    ...
    def initUI(self):
        ...
        #질문
        self.q1 = QLabel("칼퇴근이 설레는 이유는 빨리 집에 가서 쉴 수 있기 때문이다. ", self)
        self.q1.setFont(QtGui.QFont("맑은 고딕", 16))
        ...
        self.test5_question5_window = Test5_Question5_Window(self)
    def a_click(self):
        self.test5_question5_window.show()
        user_name = self.name_label.text()
        self.test5_question5_window.name_label.setText(user_name)

        print(user_name, ">>>점수 추가 완료")
        if self.label.text() == "         's Love Cell Survival Test":
            self.test5_question5_window.label.setText("         's Love Cell Survival Test")
            self.test5_question5_window.q1.setText("Getting to know new people is stressful.")
        elif self.label.text() == "         恋愛細胞生存テスト":
            self.test5_question5_window.label.setText("         恋愛細胞生存テスト")
            self.test5_question5_window.q1.setText("新しい人たちを知っていくことはストレスです。")
        self.hide()
