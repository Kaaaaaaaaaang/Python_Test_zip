...
from test5_question7_window import Test5_Question7_Window
...
class Test5_Question6_Window(QtWidgets.QMainWindow, QPushButton):
    ...
    def initUI(self):
        ...
        #질문
        self.q1 = QLabel("지나가는 커플을 보면 부러움보다 짜증이 크다. ", self)
        ...
        self.test5_question7_window = Test5_Question7_Window(self)
    def a_click(self):
        self.test5_question7_window.show()
        user_name = self.name_label.text()
        self.test5_question7_window.name_label.setText(user_name)

        print(user_name, ">>>점수 추가 완료")
        if self.label.text() == "         's Love Cell Survival Test":
            self.test5_question7_window.label.setText("         's Love Cell Survival Test")
            self.test5_question7_window.q1.setText("/ It's best to stay home without meeting anyone.")
        elif self.label.text() == "         恋愛細胞生存テスト":
            self.test5_question7_window.label.setText("         恋愛細胞生存テスト")
            self.test5_question7_window.q1.setText("誰にも会わずに家にいるのが一番だ。")
        self.hide()
