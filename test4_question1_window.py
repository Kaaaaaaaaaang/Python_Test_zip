...
from test4_question2_window import Test4_Question2_Window
...
class Test4_Question1_Window(QtWidgets.QMainWindow, QPushButton):
    ...
    def initUI(self):
        ...
        self.test4_question2_window = Test4_Question2_Window(self)

    def a_click(self):
        self.test4_question2_window.show()
        user_name = self.name_label.text()
        self.test4_question2_window.name_label.setText(user_name)

        print(user_name, ">>>점수 추가 완료")
        #print(self.name_label.text() + " : a 선택")

        sql = "UPDATE my_neologism_test SET calculate = 10 WHERE result = 0;"
        cursor.execute(sql)
        cnx.commit()
    ...
    def b_click(self):
        ...
        sql = "UPDATE my_neologism_test SET calculate = 0 WHERE result = 0;"
        cursor.execute(sql)
        cnx.commit()
