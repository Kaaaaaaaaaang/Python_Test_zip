...
from test5_question3_window import Test5_Question3_Window
...
class Test5_Question2_Window(QtWidgets.QMainWindow, QPushButton):
    ...
    def initUI(self):
        ...
        #질문
        self.q1 = QLabel("솔로 생활한 지 2년이 넘었다.", self)
        ...
        self.test5_question3_window = Test5_Question3_Window(self)
    def a_click(self):
        self.test5_question3_window.show()
        user_name = self.name_label.text()
        self.test5_question3_window.name_label.setText(user_name)

        print(user_name, ">>>점수 추가 완료")

        if self.label.text() == "         's Love Cell Survival Test":
            self.test5_question3_window.label.setText("         's Love Cell Survival Test")
            self.test5_question3_window.q1.setText("The last moment my heart raced was when I saw a picture of a cat. ")
        elif self.label.text() == "         恋愛細胞生存テスト":
            self.test5_question3_window.label.setText("         恋愛細胞生存テスト")
            self.test5_question3_window.q1.setText("私の心臓がどきどきした最後の瞬間は猫の写真を見た時だった。")
        self.hide()
